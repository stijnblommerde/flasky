...
@login_required
def post(id):
    """ permalink location of single post with comments
    """
    post = Post.query.get_or_404(id)
    form = CommentForm()
    if current_user.can(Permission.COMMENT) and form.validate_on_submit():
        comment = Comment(body=form.body.data, disabled=False,
    ...
    page = request.args.get('page', type=int, default=1)
    if page == -1:
        page = math.ceil((post.comments.count()) / current_app.config[
...
def follow(username):
    """ current_user wants to follow the user with given username
    """
    user = User.query.filter_by(username=username).first()
    if user is None:
        flash('Invalid user')
...
def unfollow(username):
    """ current_user wants to unfollow the user with given username
    """
    user = User.query.filter_by(username=username).first()
    if user is None:
        flash('Invalid user')
...
def followers(username):
    """ displays the followers of the user with given username
    """
    user = User.query.filter_by(username=username).first()
    if user is None:
        flash('Invalid user')
    ...
    pagination = user.followers.paginate(
        ...
        error_out=False)
    followers = [{'user': item.follower, 'timestamp': item.timestamp} for
...
def followed_by(username):
    """ displays the others the user with the given username is following
    """
    user = User.query.filter_by(username=username).first()
    if user is None:
        flash('Invalid user')
    ...
    pagination = user.followed.paginate(
        page=page, per_page=current_app.config['FLASKY_FOLLOWED_PER_PAGE'],
        error_out=False)
    followed = [{'user': item.followed, 'timestamp': item.timestamp} for
                 item in pagination.items]
